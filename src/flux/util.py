...
def load_flow_model(name,ckpt_path,cf_model,model_dtype,node_cr_path,use_accelerate: bool = True,use_quantize: bool = False,block_offload=True,device: str='cpu'):
    # Loading Flux
    print("Init model")
    if block_offload: #keep model on cpu
        ...
        if use_accelerate:
            ...
            model = Flux(configs[name].params).to(model_dtype)
            if use_quantize:
                json_path = os.path.join(node_cr_path,"config.json") #config is for pass block
...
def load_flow_model_(name: str, device: str, hf_download: bool = True):
    # Loading Flux
    print("Init model")
...
def load_single_condition_branch(name, device, transformer=None):
    with torch.device(device):
        condition_branch = SingleConditionBranch(configs[name].params)
    if transformer is not None:
        condition_branch.load_state_dict(transformer.state_dict(), strict=False)
...
def load_ae(name: str, device: str, ckpt_path) -> AutoEncoder:
    ...
    #         # 处理 conv_norm_out -> norm_out
    #         if "conv_norm_out" in param_name:
    #             new_name = param_name.replace("conv_norm_out.", "norm_out")
    #             sd[new_name] = sd.pop(param_name)
    #         # 处理 conv_norm -> norm
    #         elif "conv_norm" in param_name:
    #             new_name = param_name.replace("conv_norm.", "norm")
    #             sd[new_name] = sd.pop(param_name)
    #         # 处理 down_blocks -> down
    #         elif "down_blocks" in param_name:
    #             new_name = param_name.replace("down_blocks", "down")
    #             # 处理 resnets -> block
    #             if "resnets" in new_name:
    #                 new_name = new_name.replace("resnets", "block")
    #             # 处理 downsamplers -> downsample
    #             elif "downsamplers" in new_name:
    #                 new_name = new_name.replace("downsamplers", "downsample")
    #             # 处理 conv_shortcut -> nin_shortcut
    #             if "conv_shortcut" in new_name:
    #                 new_name = new_name.replace("conv_shortcut", "nin_shortcut")
    #             sd[new_name] = sd.pop(param_name)
    #         # 处理 up_blocks -> up
    #         elif "up_blocks" in param_name:
    #             new_name = param_name.replace("up_blocks", "up")
    #             # 处理 resnets -> block
    #             if "resnets" in new_name:
    #                 new_name = new_name.replace("resnets", "block")
    #             # 处理 upsamplers -> upsample
    #             elif "upsamplers" in new_name:
    #                 new_name = new_name.replace("upsamplers", "upsample")
    #             # 处理 conv_shortcut -> nin_shortcut
    #             if "conv_shortcut" in new_name:
    #                 new_name = new_name.replace("conv_shortcut", "nin_shortcut")
    #             sd[new_name] = sd.pop(param_name)
    #         # 处理 mid_block -> mid
    #         elif "mid_block" in param_name:
    #             new_name = param_name.replace("mid_block", "mid")
    #             # 处理 attentions -> attn_1
    #             if "attentions" in new_name:
    #                 new_name = new_name.replace("attentions.0", "attn_1")
    #                 # 处理 group_norm -> norm
    #                 new_name = new_name.replace("group_norm", "norm")
    #                 # 处理 to_k, to_q, to_v, to_out -> k, q, v, proj_out
    #                 new_name = new_name.replace("to_k", "k")
    #                 new_name = new_name.replace("to_q", "q")
    #                 new_name = new_name.replace("to_v", "v")
    #                 new_name = new_name.replace("to_out.0", "proj_out")
    #             # 处理 resnets -> block
    #             elif "resnets" in new_name:
    #                 new_name = new_name.replace("resnets", "block")
    #             sd[new_name] = sd.pop(param_name)
...
class WatermarkEmbedder:
    def __init__(self, watermark):
        ...
        self.encoder = None  #WatermarkEncoder()

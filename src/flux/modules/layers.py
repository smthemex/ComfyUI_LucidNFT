...
class RMSNorm(torch.nn.Module):
    ...
    def forward(self, x: Tensor):
        ...
        if x.device != self.scale.device:  # lora load on cpu
            self.scale.data = self.scale.data.to(x.device)
...
class Modulation(nn.Module):
    ...
    def forward(self, vec: Tensor) -> tuple[ModulationOut, ModulationOut]:
        if vec.device != self.lin.weight.device: # fix lora load on cpu 
            self.lin = self.lin.to(vec.device)
...
class SingleStreamBlockProcessor:
    def __call__(self, attn: nn.Module, x: Tensor, vec: Tensor, pe: Tensor) -> Tensor:
        ...
        mod, _ = attn.modulation(vec)
        x_mod = (1 + mod.scale) * attn.pre_norm(x) + mod.shift
        if x_mod.device != attn.linear1.weight.device:  # lora load on cpu
            attn.linear1 = attn.linear1.to(x_mod.device)
        qkv, mlp = torch.split(attn.linear1(x_mod), [3 * attn.hidden_size, attn.mlp_hidden_dim], dim=-1)
        ...
        if attn_1.device != attn.linear2.weight.device:  # lora load on cpu
            attn.linear2 = attn.linear2.to(attn_1.device)
